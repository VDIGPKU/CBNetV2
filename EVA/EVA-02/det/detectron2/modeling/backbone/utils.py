...
class VisionRotaryEmbeddingFast(nn.Module):
    ...
    def forward(self, t, H, W): 
        ...
        # print(t.shape, self.freqs_cos.shape, self.freqs_sin.shape)
        return  t * self.freqs_cos + rotate_half(t) * self.freqs_sin


class VisionRotaryEmbeddingFastMS(nn.Module):
    def __init__(
        self,
        dim,
        pt_seq_len=16,
        ...
        freqs_tmp = freqs
        for i, ft_seq_len_sub in enumerate(ft_seq_len):
            t = torch.arange(ft_seq_len_sub) / ft_seq_len_sub * pt_seq_len
            # print(ft_seq_len_sub, t.shape)

            freqs = torch.einsum('..., f -> ... f', t, freqs_tmp)
            freqs = repeat(freqs, '... n -> ... (n r)', r = 2)
            freqs = broadcat((freqs[:, None, :], freqs[None, :, :]), dim = -1)
            ...
            self.register_buffer(f"freqs_cos_{i}", freqs_cos)
            self.register_buffer(f"freqs_sin_{i}", freqs_sin)

            print('======== shape of rope freq', freqs_cos.shape, '========')

    def forward(self, t, H, W):
        for i in range(len(self.ft_seq_len)):
            # print(H, self.ft_seq_len[i])
            if H == self.ft_seq_len[i]:
                break
        freqs_cos = getattr(self, f'freqs_cos_{i}')
        freqs_sin = getattr(self, f'freqs_sin_{i}')
...
#         custom_freqs = self.custom_freqs
